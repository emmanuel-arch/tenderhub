...
function FileDropzone({ label, accept, hint, required = true, file, onFileSelect }: DropzoneProps) {
  return (
    <label className={`block border-2 border-dashed rounded-xl p-6 text-center transition-all duration-200 cursor-pointer ${
      ...
      {file ? (
        ...
        <>
          ...
          <div className="font-medium text-slate-700 mb-1">{label}{required ? ' *' : ' (Optional)'}</div>
